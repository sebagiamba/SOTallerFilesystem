...
struct Ext2FSInode *Ext2FS::load_inode(unsigned int inode_number) {
    // Obtengo el número de grupo al cual pertenece el indodo numero inode_number.
    unsigned int group_number = blockgroup_for_inode(inode_number);

    // Obtengo el índice en la tabla de inodos del grupo group_number.
    unsigned int inode_index = blockgroup_inode_index(inode_number);

    // Obtengo el descriptor de grupo.
    Ext2FSBlockGroupDescriptor *group_desc = block_group(group_number);

    // Calculo el número de bloque que voy a leer del disco.
    // (la función read_block no lee un inodo sino un bloque completo)
    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned int inode_size = _superblock->inode_size;
    unsigned int inodes_per_block = block_size / inode_size;
    unsigned int block_number = inode_number / inodes_per_block;

    // Obtengo la dirección de memoria (en disco) del bloque que quiero leer.
    unsigned int block_address = group_desc->inode_table + (block_number * block_size);

    // Creo un buffer y leo el bloque.
    auto *buffer = (unsigned char*)(malloc(sizeof(unsigned char)));
    read_block(block_address, buffer);

    auto *ret_inode = (Ext2FSInode *)(buffer + inode_index * inode_size);
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode *inode, unsigned int block_number) {
    unsigned int BLOQUE_INDIRECTO_1 = 12;
    unsigned int BLOQUE_INDIRECTO_2 = 13;
    unsigned int BLOQUE_INDIRECTO_3 = 14;

    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned int CANT_ENTRADAS_BLOQUE = block_size / 4;
    ...
    if (block_number < CANT_ENTRADAS_BLOQUE)
        return inode->block[BLOQUE_INDIRECTO_1] + block_number * block_size;

    // Normalizo
    block_number -= CANT_ENTRADAS_BLOQUE;

    // Bloque indirecto doble
    if (block_number < (CANT_ENTRADAS_BLOQUE^2)) {
        unsigned int entrada_bloque_doble = block_number / CANT_ENTRADAS_BLOQUE;
        unsigned int entrada_bloque_simple = block_number % CANT_ENTRADAS_BLOQUE;

        return inode->block[BLOQUE_INDIRECTO_2] + (entrada_bloque_doble * block_size) +
               (entrada_bloque_simple * block_size);
    }

    // Normalizo
    block_number -= (CANT_ENTRADAS_BLOQUE^2);

    // Bloque indirecto triple
    if (block_number < (CANT_ENTRADAS_BLOQUE^3)) {
        unsigned int entrada_bloque_triple = block_number / (CANT_ENTRADAS_BLOQUE^2);
        unsigned int entrada_bloque_doble = block_number / CANT_ENTRADAS_BLOQUE;
        unsigned int entrada_bloque_simple = block_number % CANT_ENTRADAS_BLOQUE;

        return inode->block[BLOQUE_INDIRECTO_3] + (entrada_bloque_triple * block_size) +
               (entrada_bloque_doble * block_size) + (entrada_bloque_simple * block_size);
    }
}
...
struct Ext2FSInode *Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode *from, const char *filename) {
    ...
    // TODO: Chequear si from.blocks devuelve la cantidad de bloques de "datos" total u ocupada (o si devuelve fruta).
    for (int i = 0; i < from->blocks; i++) {
        dir_entry = (Ext2FSDirEntry *) get_block_address(from, i);
        if (!DENT_ISDIR(dir_entry) && dir_entry->name == filename)
...
int Ext2FS::read(fd_t filedesc, unsigned char *buffer, int size) {
    int realsize = ((_seek_memory[filedesc] + size) >= _open_files[filedesc].size) ? _open_files[filedesc].size -
                                                                                     _seek_memory[filedesc] : size;
    if (realsize > 0) {
