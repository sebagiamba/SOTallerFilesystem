...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
    ...
    // Busco la dirección de memoria del bloque del inodo. (TAM_INODO -> superbloque)
    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned short inode_size = _superblock->inode_size;
    int inodes_per_block = block_size / inode_size;
    int block_number = inode_number / inodes_per_block;
    int inode_index = inode_number % inodes_per_block;

    int inode_block_address = group_desc.inode_table + block_number * block_size;

    unsigned int* inode_block = malloc(sizeOf(unsigned int));
    read_block(inode_block_address, inode_block);

    // Reservo memoria para el inodo a cargar.
    Ext2FSInode* ret_inode = malloc(sizeOf(Ext2FSInode));
    ret_inode = inode_block + inode_index * inode_size;
...
{
    int BLOQUE_INDIRECTO_1 = 12;
    int BLOQUE_INDIRECTO_2 = 13;
    int BLOQUE_INDIRECTO_3 = 14;

    unsigned int block_size = 1024 << _superblock->log_block_size;
    int CANT_ENTRADAS_BLOQUE = block_size / 4;
    ...
    if (block_number < CANT_ENTRADAS_BLOQUE)
        return inode.block[BLOQUE_INDIRECTO_1][block_number];
    ...
    if (block_number < CANT_ENTRADAS_BLOQUE^2) {
        int entrada_bloque_doble = block_number / CANT_ENTRADAS_BLOQUE;
        int entrada_bloque_simple = block_number % CANT_ENTRADAS_BLOQUE;

        return inode.block[BLOQUE_INDIRECTO_2][entrada_bloque_doble][entrada_bloque_simple];
    }
    ...
    if (block_number < CANT_ENTRADAS_BLOQUE^3) {
        int entrada_bloque_triple = block_number / CANT_ENTRADAS_BLOQUE^2;
        int entrada_bloque_doble = block_number / CANT_ENTRADAS_BLOQUE;
        int entrada_bloque_simple = block_number % CANT_ENTRADAS_BLOQUE;

        return inode.block[BLOQUE_INDIRECTO_3][entrada_bloque_triple][entrada_bloque_doble][entrada_bloque_simple];
    }
}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	for (int i = 0; i < from.blocks; i++) {
        dir_entry = get_block_address(from, i);
